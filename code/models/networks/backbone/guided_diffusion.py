...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.num_classes > 0:
            if multitags:
                self.class_embed = nn.Linear(self.num_classes, cond_embed_dim)
            else:
                self.class_embed = nn.Sequential(
                    OneHot(self.num_classes, exclude_zero=True),
                    nn.Linear(self.num_classes, cond_embed_dim)
                )
    ...
    def forward(self, x, gammas, y=None):
        ...
        :param y: a 1-D batch of class indices.
        :return: an [N x C x ...] Tensor of outputs.
        """
        hs = []
        gammas = gammas.view(-1, )
        time_emb = self.cond_embed(gamma_embedding(gammas, self.inner_channel))
        if self.num_classes and y is not None:
            if self.multitags:
                assert y.ndim == 2
                y = y.div(torch.count_nonzero(y, dim=1).clamp(min=1.).sqrt().unsqueeze(1))
            emb = time_emb + self.class_embed(y)
        else:
            emb = time_emb
...
def guided_diffusion(in_channel, out_channel, **kwargs):
    '''
    Adapted from https://github.com/Janspiry/DiffGuard-Image-to-Image-Diffusion-Models,
    an unofficial implementation of NeurIPS 2021 paper: Diffusion Models Beat GANS on Image Synthesis.
    '''
    return UNet(in_channel=in_channel, out_channel=out_channel, **kwargs)